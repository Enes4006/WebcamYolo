...
namespace WinFormsApp3
{
    ...
    {
        //private List<YoloItem> lastDetectedItems = new List<YoloItem>();

        //private readonly int detectionHistoryLength = 10;
        //private Queue<List<YoloItem>> detectionHistory = new Queue<List<YoloItem>>();
        public Form1()
        {
            InitializeComponent();
        ...
        private List<YoloItem> DetectObjects(Bitmap img)
        {
            using (var memoryStream = new MemoryStream())
            {
                img.Save(memoryStream, System.Drawing.Imaging.ImageFormat.Png);
                return yolo.Detect(memoryStream.ToArray()).ToList();
            }
        }
        private void DrawDetectionsOnImage(Bitmap img, List<YoloItem> items)
        {
            using (var graphics = Graphics.FromImage(img))
            {
                ...
                foreach (YoloItem item in items)
                {
                    ...
                    graphics.DrawString(item.Type, font, brush, item.X, item.Y);
                }
            }
        }

        //private void UpdateDetectionHistoryPanel()
        //{
        //    listBox1.Items.Clear();
        //    int frameNum = frameCount - detectionHistory.Count * countX;
        //    foreach (var detectionList in detectionHistory)
        //    {
        //        foreach (var item in detectionList)
        //        {
        //            listBox1.Items.Add($"Kare {frameNum}: {item.Type} ({Math.Round(item.Confidence, 2)})");
        //        }
        //        frameNum += countX;
        //    }
        //}
        ...
        //    if (frameCount % countX == 0)
        //    {
        //        // Yeni tespit yap
        //        lastDetectedItems = DetectObjects(img);
        //        this.Invoke(new Action(() => {
        //            ResetSource();
        //            GetSource(lastDetectedItems);
        //        }));
        //    }

        //    // Son tespit edilen kutularý her karede çiz
        //    DrawDetectionsOnImage(img, lastDetectedItems);
        ...
        //        // Geçmiþe ekle
        //        detectionHistory.Enqueue(new List<YoloItem>(lastDetectedItems));
        //        if (detectionHistory.Count > detectionHistoryLength)
        //            detectionHistory.Dequeue();

        //        this.Invoke(new Action(() => {
        //            ResetSource();
        //            GetSource(lastDetectedItems);
        //            UpdateDetectionHistoryPanel();
        //        }));
        //    }
        //}
